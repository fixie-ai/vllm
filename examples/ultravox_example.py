...
def run_ultravox():
    ...
    processed = audio_processor(audio=audio,
                                sampling_rate=16000,
                                return_tensors="pt",
                                padding="longest")
    ...
    prompt = prompt.replace("<|audio|>",
                            '<|reserved_special_token_0|>' * audio_num_tokens)
    text_input = tokenizer.apply_chat_template(conversation=[{
        "role": "user",
        "content": prompt
    }],
                                               tokenize=False,
    ...
    multi_modal_data = MultiModalData(type=MultiModalData.Type.AUDIO,
                                      data=audio_values)
    print(
        f"Loaded audio with len {audio_values.shape} and dtype {audio_values.dtype}"
    )
